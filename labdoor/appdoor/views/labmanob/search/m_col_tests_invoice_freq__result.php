...
<div class="uk-panel uk-margin-small uk-card uk-card-default uk-card-small uk-width-expand">
    ...
    <div class="uk-margin">
        <h3>Tests Invoice Frequency</h3> 
        <?php 
        echo (isset($total_search_results)) ? '<span class="uk-text-meta"> '.$total_search_results.' results found</span>' : '' ; 
        echo (isset($total_search_results) && isset($total_test_quantity)) ? '<span class="uk-text-meta"> and </span>' : '' ;
        echo (isset($total_test_quantity)) ? '<span class="uk-text-meta"> this page total test quantity '.$total_test_quantity.'</span>' : '' ; 
        ?>
        <div class="uk-margin-small">
            <?php 
            if ((isset($tests_invoice_freq_table)) && (!$tests_invoice_freq_table_empty)) 
            { 
                echo '<div class="uk-overflow-auto">'; 
                echo $tests_invoice_freq_table; 
                echo '</div>'; 
            } 
            else 
            { 
