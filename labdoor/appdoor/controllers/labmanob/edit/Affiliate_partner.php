...
class Affiliate_partner extends CI_Controller 
{ 
            ...
            if(strtolower($_SESSION['username']) != 'viewer')
            {
                ...
                if(isset($id) && ($id_input_ok === TRUE))
                {
                    if($this->labmanob_view_model->count_affiliate_partners_by_id($id) > 0)
                    {
                        ...
                        if ($this->form_validation->run() == FALSE) 
                        {
                            ...
                            $h_view_data['title'] = "Edit Affiliate Partner"; 
                            ...
                            $this->load->view('labmanob/edit/affiliate_partner/m_col_main', $view_data); 
                        ...
                        else 
                        {
                            ...
                            if ($this->labmanob_edit_model->affiliate_partner($db_data, $id) === TRUE) 
                            { 
                                // ALL Done and Success
                                // log the username who updated the information
                                log_message('error', '===== EDIT ACTION INFORMATION =====> Username: '.$_SESSION['username'].' ----- User ID: '.$_SESSION['user_id'].' ----- Affiliate Partner ID: '.$id.' ----- Updated Affiliate Partner info.');

                                $this->session->set_flashdata('message_display', 'Affiliate Partner Updated!'); 
                                redirect('labmanob/edit/affiliate_partner/index/'.$affiliate_partner->id, 'refresh');
                            ...
                            else
                            {
                                ...
                                $view_data['affiliate_partner'] = $affiliate_partner;
                                $view_data['error_message'] = "Some error occurred while updating affiliate partner database! Try again.";
                ...
                else
                {
                    ...
                    if ($this->form_validation->run() == FALSE) 
                    {
                        ...
                        $h_view_data['title'] = "Select Affiliate Partner | Edit Affiliate Partner"; 
                        $view_data['summary'] = $this->labmanob_summary_model->get_full_summary(); 
                        
                        $this->load->view('header', $h_view_data); 
                        $this->load->view('labmanob/l_col_main', $view_data); 
                        $this->load->view('labmanob/edit/affiliate_partner/m_col_select_affiliate_partner', $view_data);
                        $this->load->view('labmanob/r_col_main', $view_data); 
                        $this->load->view('footer', $f_view_data);
                    }
                    else
                    {
                        redirect('labmanob/edit/affiliate_partner/index/'.$this->input->post('affiliate_partner_id', TRUE), 'refresh');
                    }
                }
            }
            else 
